...
module.exports = async function(payload) {
	try {
		...
		try {
			...
			this.broker.emit("appointments.appointment.confirmed", {
				actor: { id: null, role: "system" },
				action: "appointments.appointment.confirmed",
				entity_type: "appointment",
				entity_id: appt.id,
				status: "ok",
				metadata: {
			...
			this.logger.info("payments.payment.completed -> appointment confirmed", { apptId });
		} catch (e) {
			this.logger.error("Failed to confirm appointment from payment.completed", { id: appt.id, message: e.message });
			this.broker.emit("logs.record", {
				actor: { id: null, role: "system" },
				action: "appointments.appointment.setStatus",
				entity_type: "appointment",
		...
		this.logger.error("payments.payment.completed handler error", { message: err && err.message });
	}
};
